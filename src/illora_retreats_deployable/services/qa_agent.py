...
class ConciergeBot:
    ...
    def ask(self, query: str, user_type: Optional[str] = None) -> str:
        try:
            logger.info(f"[ask] Received query: {query} | user_type={user_type}")
            ...
            logger.debug("[ask] Retrieving docs...")
            docs = self.retriever.invoke(query)
            logger.info(f"[ask] Retrieved {len(docs) if docs else 0} docs")
    
            system_prompt = self._build_prompt(hotel_data="\n\n".join(d.page_content for d in docs) if docs else "",
                                               query=query)
            logger.debug(f"[ask] System prompt built: {system_prompt[:500]}...")
    
            logger.debug("[ask] Invoking LLM...")
            response = self.llm.invoke([{"role": "user", "content": system_prompt}])
            logger.debug(f"[ask] LLM raw response: {response}")
